...
try:
    ...
    QR_AVAILABLE = False
    print("⚠️ Install qrcode[pil] for QR terminal output: pip install qrcode[pil]")
...
async def qr_login_terminal():
    ...
    print_neon("Scan the QR code below with your Telegram app:", 'cyan')
    try:
        qr = await client.qr_login()
        if QR_AVAILABLE:
            img = qrcode.make(qr.url)
            img.save("qr_login.png")
            print_neon(f"✅ QR code also saved as 'qr_login.png'", 'green')
            # Print ASCII QR
            try:
                # Convert to ASCII
        ...
        print_neon("Waiting for login...", 'cyan')
        try:
